...
class GeneralRandom:
  ...
  def __init__(self, x, p = None, Nrl = 1000):
  ...
  def set_pdf(self, x, p, Nrl = 1000):
    ...
    self.Nrl = Nrl
    #place y between minimum and maximum x values
    self.delta = 1.0/Nrl
    self.y = numpy.arange(0.0,1.0,self.delta)

    self.inversecdf = numpy.zeros(Nrl)    
    self.inversecdf[0] = self.x[0]
    cdf_idx = 0
    for n in xrange(1,self.inversecdfbins):
      while self.cdf[cdf_idx] < self.y[n] and cdf_idx < Nrl:
        cdf_idx += 1
      self.inversecdf[n] = self.x[cdf_idx]#self.x[cdf_idx-1] + (self.x[cdf_idx] - self.x[cdf_idx-1]) * (y[n] - self.cdf[cdf_idx-1])/(self.cdf[cdf_idx] - self.cdf[cdf_idx-1]) 
      if cdf_idx >= Nrl:
        break
